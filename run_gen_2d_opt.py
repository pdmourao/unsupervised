...
m_values = np.linspace(1, 50, num = num_points, endpoint = True, dtype = int)
...
experiment = lab.Experiment(directory = 'Data_remote', func = exp.gen_optimal, m_values = m_values, r_values = r_values,
                            neurons = neurons, rank = rank, p = p, reduced = reduced, diagonal = diagonal,
                            initial = initial, max_it = max_it)
m_arc, m_ex, its, errors = experiment.read()

print(np.mean(m_arc, axis = 0))
